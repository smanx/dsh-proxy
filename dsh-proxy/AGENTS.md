...
- Dev type sources come from the npm registry, pinned to the versions the current harness (`@deepseek-ai/dsh@0.1.0-rc.6`) ships internally: `@deepseek-ai/cordis@^4.0.1`, `@deepseek-ai/schemastery@^3.18.1`, and the `@deepseek-ai/dsh-*` packages at exact `0.1.0-rc.6`. When the harness version moves, bump these to the new harness's bundled versions and re-run `pnpm install` to fix API drift.
