...
- This plugin lives at `dsh-lan-proxy/` inside the dsh-proxy repository (the DSH-plugin variant, alongside the standalone `go/` and `node/` builds). The repo ROOT carries a thin package manifest (name `dsh-lan-proxy`) so the plugin installs from the repository itself: online `dsh plugin --profile <name> add github:smanx/dsh-proxy#main` (recommended) or local `dsh plugin --profile <name> add file:C:/mydata/codes/dsh-proxy` (bundle route); changes take effect on the **next `dsh web` restart**.
